...
<form method="post" class="mt-3 mb-5">
    ...
    {{ tag.inputSubmit('search', 'Search', ['class': 'btn btn-primary']) }}
    ...
    {% if request.isPost() and profile %}
        ...
        <div class="mt-3 mb-5">
                    ...
                    {% for action in actions %}
                        <tr>
                            <td class="text-center">
                                <input type="checkbox"
                                       name="permissions[]"
                                       value="{{ resource ~ '.' ~ action }}" {% if permissions[resource ~ '.' ~ action] is defined %} checked="checked" {% endif %}>
                            </td>
            ...
            {{ tag.inputSubmit('Submit', 'submit', ['class': 'btn btn-primary']) }}
        </div>
    {% endif %}
</form>
